...
def mock_steam_provider():
    ...
    provider.get_price_overview = AsyncMock(return_value={
        "success": True,
        "lowest_price": "$1.23",
        "median_price": "$2.34",
        "volume": "100"
    ...
    provider.get_inventory = AsyncMock(return_value={
        "success": True,
        "rgInventory": {},
        "rgDescriptions": {}
    })
...
def mock_send_telegram_message():
    ...
    with patch('app.services.notifier.send_telegram_message', new_callable=AsyncMock) as mock:
        yield mock
