...
from app.db.models import Item, ItemSnapshot, PriceAlert, Subscription, User
...
async def test_check_price_alerts_triggers(mock_session, mock_send_telegram_message):
    alert = PriceAlert(
        id=1, user_id=1, item_id=1, percent_change=10.0, period="24h", active=True
    )
...
@pytest.mark.asyncio
async def test_check_price_alerts_no_trigger_small_change(
    mock_session, mock_send_telegram_message
):
    alert = PriceAlert(
        id=1, user_id=1, item_id=1, percent_change=50.0, period="24h", active=True
    )
...
async def test_send_daily_digest(mock_session, mock_send_telegram_message):
    ...
    snapshot = ItemSnapshot(
        item_id=1, median_price=150.0, price_24h_ago=140.0, trend_direction="up"
    )
...
async def test_check_price_alerts_weekly(mock_session, mock_send_telegram_message):
    alert = PriceAlert(
        id=1, user_id=1, item_id=1, percent_change=10.0, period="weekly", active=True
    )
...
@pytest.mark.asyncio
async def test_check_price_alerts_old_price_zero(
    mock_session, mock_send_telegram_message
):
    alert = PriceAlert(
        id=1, user_id=1, item_id=1, percent_change=10.0, period="24h", active=True
...
@pytest.mark.asyncio
async def test_check_price_alerts_inactive_alert(
    mock_session, mock_send_telegram_message
):
    alert = PriceAlert(
        id=1, user_id=1, item_id=1, percent_change=10.0, period="24h", active=False
    )
    mock_session.execute.return_value.scalars.return_value.all.return_value = [alert]
...
async def test_send_digests_weekly(mock_session, mock_send_telegram_message):
    ...
    snapshot = ItemSnapshot(
        item_id=1, median_price=150.0, price_24h_ago=None, trend_direction="up"
    )
    user = User(id=1, chat_id=789)
