...
from app.db.models import Item, ItemSnapshot, ItemDailyStats, ItemHourlyStats
...
async def test_update_snapshots_success(mock_session, mock_steam_client, mock_redis):
    ...
    mock_steam_client.get_price_overview = AsyncMock(return_value={
        "success": True,
        "lowest_price": "$5.00",
        "median_price": "$6.50",
        "volume": "200"
    ...
    with patch('app.state.redis_client', mock_redis), \
         ...
         patch('app.services.collector.compute_trend', new_callable=AsyncMock) as mock_trend, \
         patch('app.services.collector.invalidate_price_chart_cache', new_callable=AsyncMock) as mock_invalidate:
...
async def test_aggregate_hourly_to_daily(mock_session):
    # Arrange
    hourly_records = [
        ItemHourlyStats(item_id=1, timestamp=datetime(2026,7,20,10,0), price=10.0, volume=100),
        ItemHourlyStats(item_id=1, timestamp=datetime(2026,7,20,11,0), price=11.0, volume=150),
        ItemHourlyStats(item_id=1, timestamp=datetime(2026,7,21,10,0), price=12.0, volume=200),
    ]
    mock_session.execute.return_value.scalars.return_value.all.return_value = hourly_records
...
async def test_get_price_24h_ago_daily(mock_session):
    # Arrange
    mock_session.execute.return_value.scalar_one_or_none.side_effect = [None, 9.8]  # часовая -> None, дневная -> 9.8
...
async def test_ensure_item_history_existing_data(mock_session, mock_steam_client):
    mock_session.scalar = AsyncMock(side_effect=[10, 5])  # hourly_count=10, daily_count=5
...
async def test_ensure_item_history_loads_data(mock_session, mock_steam_client):
    ...
    with patch('app.services.collector.aggregate_hourly_to_daily', new_callable=AsyncMock) as mock_aggregate:
        await collector.ensure_item_history(mock_session, mock_steam_client, item)
...
async def test_update_single_item_snapshot_success(mock_session, mock_steam_client):
    ...
    mock_steam_client.get_price_overview = AsyncMock(return_value={
        "success": True, "lowest_price": "$5.00", "median_price": "$6.50", "volume": "200"
    })
    mock_session.get = AsyncMock(return_value=None)
    with patch('app.services.collector.compute_trend', new_callable=AsyncMock) as mock_trend:
        mock_trend.return_value = (0.1, "up")
        result = await collector.update_single_item_snapshot(mock_session, mock_steam_client, item)
