...
async def test_restore_or_create_from_redis(session_manager):
    ...
    with patch('app.services.steam.session_manager.SteamSession.deserialize', return_value=MagicMock(cookies_are_valid=True)):
...
async def test_create_new_session(session_manager):
    ...
    with patch('app.services.steam.session_manager.SteamSession') as MockSession, \
         patch('app.services.steam.session_manager.retry_async', new=lambda func, *a, **k: func()):
...
async def test_restore_or_create_invalid_cookies_refresh_fails(session_manager):
    ...
    with patch('app.services.steam.session_manager.SteamSession.deserialize') as mock_deser:
        mock_session = MagicMock(cookies_are_valid=False)
        mock_session.refresh_access_token = AsyncMock(side_effect=Exception("refresh failed"))
        mock_deser.return_value = mock_session
        with patch.object(session_manager, '_create_new_session', new_callable=AsyncMock) as mock_create:
            await session_manager._restore_or_create()
            mock_create.assert_called_once()

@pytest.mark.asyncio
async def test_load_guard_account_file_not_found(session_manager, tmp_path, monkeypatch):
...
async def test_create_new_session_with_guard_confirmation(session_manager):
    ...
    with patch('app.services.steam.session_manager.SteamSession') as MockSession, \
         patch('app.services.steam.session_manager.retry_async', new=lambda func, *a, **k: func()):
        mock_session = MockSession.return_value
        mock_session.with_credentials = AsyncMock(side_effect=GuardConfirmationRequired(confirmations=[], 
                                                                                        allowed_guard_types=[]))
        ...
        with patch.object(session_manager, '_load_guard_account', return_value=MagicMock(shared_secret=MagicMock(generate_auth_code=MagicMock(return_value='ABC')))):
            await session_manager._create_new_session()
