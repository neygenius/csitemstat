...
from app.bot.dispatcher import (
    cmd_start, cmd_help, cmd_link_steam, process_steam_id,
    cmd_inventory, cb_inventory_page, cb_add_track, cmd_track,
    cmd_portfolio, cb_portfolio_page, cb_tracked_item, cmd_stats,
    cb_stats, cb_price_period, send_price_chart, cb_sub_add, cb_subs,
    cb_tracked_subs, cb_sub_remove, cb_tracked_alerts, cb_alert_remove,
    cb_alert_add_start, process_alert_percent, cb_alert_period,
    cb_tracked_untrack, reply_use_help, cmd_subscribe, cmd_alert,
    cmd_untrack, cmd_unsubscribe, cmd_delete_alert,
)
from app.db.models import (
    User, Item, UserTrackedItem, ItemDailyStats, ItemHourlyStats, 
    ItemSnapshot, Subscription, PriceAlert
)
...
def mock_app_state():
    ...
    steam_client.get_inventory = AsyncMock(return_value={"success": True, "rgInventory": {}, "rgDescriptions": {}})
    steam_client.close = AsyncMock()

    with patch('app.bot.dispatcher.app_state.cleanup_manager', cleanup_manager), \
...
@pytest.mark.asyncio
async def test_cmd_start_creates_new_user(mock_db, mock_bot, mock_message, mock_app_state):
...
@pytest.mark.asyncio
async def test_cmd_help_registers_temp_message(mock_db, mock_bot, mock_message, mock_app_state):
    msg_mock = AsyncMock()
...
@pytest.mark.asyncio
async def test_cmd_link_steam_sets_state(mock_db, mock_bot, mock_message, mock_state, mock_app_state):
...
@pytest.mark.asyncio
async def test_process_steam_id_invalid(mock_db, mock_bot, mock_message, mock_state, mock_app_state):
...
@pytest.mark.asyncio
async def test_process_steam_id_success(mock_db, mock_bot, mock_message, mock_state, mock_app_state):
...
@pytest.mark.asyncio
async def test_cmd_inventory_without_steam(mock_db, mock_bot, mock_message, mock_app_state):
...
@pytest.mark.asyncio
async def test_cmd_inventory_with_cache(mock_db, mock_bot, mock_message, mock_app_state):
...
@pytest.mark.asyncio
async def test_cmd_inventory_fetch_error(mock_db, mock_bot, mock_message, mock_app_state):
    user = User(id=123456789, chat_id=123456789, steam_id64=b"encrypted")
    mock_db.get = AsyncMock(return_value=user)
    mock_app_state['steam_client'].get_inventory = AsyncMock(side_effect=Exception("Steam error"))
    with patch('app.bot.dispatcher.decrypt_steam_id', return_value=76561198000000000):
        await cmd_inventory(mock_message)
...
@pytest.mark.asyncio
async def test_cmd_stats_multiple_items(mock_db, mock_bot, mock_message, mock_app_state):
...
@pytest.mark.asyncio
async def test_cmd_stats_snapshot_exists(mock_db, mock_bot, mock_message, mock_app_state):
    mock_message.text = "/stats AK-47"
    item = Item(id=1, app_id=730, market_hash_name="AK-47", name="AK-47")
    snapshot = ItemSnapshot(item_id=1, median_price=100.0, lowest_price=90.0, volume_24h=100,
                            trend_direction="up", price_24h_ago=95.0)
    mock_db.execute.return_value.scalars.return_value.all.return_value = [item]
    mock_db.get = AsyncMock(return_value=snapshot)
    with patch('app.bot.dispatcher.item_actions') as mock_actions, \
         patch('app.bot.dispatcher.send_price_chart', new_callable=AsyncMock) as mock_chart:
...
@pytest.mark.asyncio
async def test_cmd_stats_no_snapshot_success(mock_db, mock_bot, mock_message, mock_app_state):
    mock_message.text = "/stats AK-47"
    item = Item(id=1, app_id=730, market_hash_name="AK-47", name="AK-47")
    mock_db.execute.return_value.scalars.return_value.all.return_value = [item]
    ...
    with patch('app.bot.dispatcher.update_single_item_snapshot', new_callable=AsyncMock) as mock_update, \
         patch('app.bot.dispatcher.send_price_chart', new_callable=AsyncMock) as mock_chart:
...
@pytest.mark.asyncio
async def test_cb_add_track_existing(mock_db, mock_bot, mock_callback_query, mock_app_state):
...
@pytest.mark.asyncio
async def test_cb_inventory_page(mock_db, mock_bot, mock_callback_query, mock_app_state):
...
async def test_cb_stats(mock_db, mock_bot, mock_callback_query, mock_app_state):
    ...
    with patch('app.bot.dispatcher.cmd_stats', new_callable=AsyncMock) as mock_cmd_stats:
        await cb_stats(mock_callback_query)
        mock_cmd_stats.assert_called_once_with(mock_callback_query.message, item.market_hash_name)
...
async def test_cb_sub_add(mock_db, mock_bot, mock_callback_query, mock_app_state):
    ...
    user = User(id=123456789, chat_id=123456789)
    item = Item(id=1, app_id=730, market_hash_name="AK-47")
    mock_db.get = AsyncMock(side_effect=lambda model, pk: user if model == User else item)
...
@pytest.mark.asyncio
async def test_cb_portfolio_page(mock_db, mock_bot, mock_callback_query, mock_app_state):
...
async def test_cb_price_period(mock_db, mock_bot, mock_callback_query, mock_app_state):
    ...
    with patch('app.bot.dispatcher.send_price_chart', new_callable=AsyncMock) as mock_chart:
...
@pytest.mark.asyncio
async def test_cb_tracked_subs_no_subs(mock_db, mock_bot, mock_callback_query, mock_app_state):
...
@pytest.mark.asyncio
async def test_cb_tracked_subs_with_subs(mock_db, mock_bot, mock_callback_query, mock_app_state):
    mock_callback_query.data = "tracked_subs:1"
    user = User(id=123456789, chat_id=123456789)
...
@pytest.mark.asyncio
async def test_cb_tracked_alerts_no_alerts(mock_db, mock_bot, mock_callback_query, mock_app_state):
...
@pytest.mark.asyncio
async def test_cb_alert_add_start(mock_db, mock_bot, mock_callback_query, mock_state, mock_app_state):
...
@pytest.mark.asyncio
async def test_process_alert_percent_invalid(mock_db, mock_bot, mock_message, mock_state, mock_app_state):
...
@pytest.mark.asyncio
async def test_process_alert_percent_valid(mock_db, mock_bot, mock_message, mock_state, mock_app_state):
...
@pytest.mark.asyncio
async def test_cb_alert_period(mock_db, mock_bot, mock_callback_query, mock_state, mock_app_state):
...
@pytest.mark.asyncio
async def test_cb_tracked_untrack(mock_db, mock_bot, mock_callback_query, mock_app_state):
    mock_callback_query.data = "tracked_untrack:1"
    user = User(id=123456789, chat_id=123456789)
    tracked = UserTrackedItem(user_id=123456789, item_id=1)
    mock_db.get = AsyncMock(side_effect=lambda model, pk: user if model == User else tracked)
...
async def test_cmd_subscribe(mock_db, mock_bot, mock_message, mock_app_state):
    with patch('app.bot.dispatcher.reply_use_help', new_callable=AsyncMock) as mock_reply:
...
async def test_send_price_chart_from_cache(mock_db, mock_bot, mock_app_state):
    # Мокаем redis, чтобы вернуть кэш
...
async def test_send_price_chart_generates_new(mock_db, mock_bot, mock_app_state):
    ...
    with patch('app.bot.dispatcher.generate_price_chart', return_value=b"chart"), \
...
async def test_send_price_chart_hourly(mock_db, mock_bot, mock_app_state):
    ...
    mock_db.get = AsyncMock(return_value=item)
    hourly = [
        ItemHourlyStats(item_id=1, timestamp=datetime.now(timezone.utc) - timedelta(hours=2), price=10.0, volume=100),
        ItemHourlyStats(item_id=1, timestamp=datetime.now(timezone.utc) - timedelta(hours=1), price=11.0, volume=120)
    ]
...
async def test_send_price_chart_daily_all_time(mock_db, mock_bot, mock_app_state):
    ...
    mock_db.get = AsyncMock(return_value=item)
    daily = [
        ItemDailyStats(item_id=1, date=datetime.now().date() - timedelta(days=2), price=10.0, volume=100),
        ItemDailyStats(item_id=1, date=datetime.now().date() - timedelta(days=1), price=11.0, volume=120)
    ]
...
async def test_send_price_chart_no_records(mock_db, mock_bot, mock_app_state):
    ...
    mock_db.get = AsyncMock(return_value=item)
    mock_db.execute.return_value.scalars.return_value.all.return_value = []
    with patch('app.bot.dispatcher.generate_price_chart') as mock_gen, \
         patch('app.bot.dispatcher.bot', mock_bot):
        mock_bot.send_photo = AsyncMock()
        await send_price_chart(chat_id=123456789, item_id=1, days=7)
