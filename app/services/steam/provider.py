...
class SteamProvider(ISteamProvider):
    ...
    async def get_price_overview(self, app_id: int, market_hash_name: str) -> Dict[str, Any]:
        try:
            async def _fetch():
                return await self._public_client.market.get_price_overview(market_hash_name, App(app_id))
            ...
            return {
                "success": True,
                "lowest_price": f"{overview.lowest_price / 100:.2f}" if overview.lowest_price is not None else "0.00",
    ...
    async def get_price_history(self, app_id: int, market_hash_name: str) -> List[List]:
        if not await self.ensure_authenticated():
            logger.error(f"Authentication failed for fetch {market_hash_name} history", exc_info=True)
            return []
        
        try:
            async def _fetch():
                return await self._client.market.get_price_history(market_hash_name, App(app_id))
    ...
    async def get_inventory(self, steam_id64: int, app_id: int) -> Dict[str, Any]:
        if not await self.ensure_authenticated():
            logger.error(f"Authentication failed for fetch inventory {steam_id64}", exc_info=True)
            return {"success": False}
        ...
        try:
            ...
            async def _fetch():
                return await self._client.inventory.get_user_inventory(user_id, 
                                                                       AppContext(App(app_id), 2))
